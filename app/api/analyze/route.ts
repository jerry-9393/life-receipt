...
export async function POST(req: Request) {
  try {
    ...
    1. **창의적 표현 필수:** "Lv.999", "바닥", "없음" 같은 식상한 표현 절대 금지.
    2. **MZ식 드립과 비유 활용:** 게임 용어, 인터넷 밈, 현실 비유를 섞어서 매번 다르게 표현하세요.
       - (X) "Lv.999" → (O) "만렙 찍음", "보스급", "레전드", "갓", "킹", "신급"
       - (X) "바닥" → (O) "지하실", "마이너스", "0.1%", "나락", "헬게이트"
       - (X) "없음" → (O) "품절", "매진", "N/A", "공백", "빈칸"
    3. **항상 똑같은 표현 금지:** 매번 다른 창의적인 표현을 사용하세요. 같은 패턴 반복 금지.
    ...
    [출력 JSON 포맷]
    {
      "receipt_header": {
        ...
        "items": [
           {"name": "고집", "qty": "보스급", "price": "한 번 꽂히면 감"},
           {"name": "실행력", "qty": "나락", "price": "마감 직전에 함"},
           {"name": "눈치", "qty": "공감각 발달", "price": "상황 파악 빠름"},
