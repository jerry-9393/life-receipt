...
export async function POST(req: Request) {
  try {
    ...
    2. **행동으로 번역 필수:** 반드시 구체적인 행동이나 상황을 6~12글자 한 줄로 표현하세요.
       ...
       - (X) "별로" → (O) "돈 들어오면 씀"
       - (X) "굿" → (O) "연락은 필요할 때만"
    3. **item.name에 맞춰 작성:** 각 항목의 특성에 맞는 구체적인 행동을 묘사하세요.
       - 예: "고집" → "남 말 안 들음", "계획 고치기 싫어함"
       - 예: "실행력" → "계획만 세우고 안 함", "시작은 잘함"
       - 예: "눈치" → "눈치 보긴 함", "분위기 못 읽음"
    ...
    [출력 JSON 포맷]
    {
      "receipt_header": {
        ...
        "items": [
           {"name": "고집", "qty": "Lv.999", "price": "남 말 안 들음","한 번 꽂히면 감"},
           {"name": "실행력", "qty": "바닥", "price": "계획만 세우고 안 함","마감 직전에 함"},
           {"name": "눈치", "qty": "없음", "price": "눈치 보긴 함","상황 파악 빠름"}
           {"name": "자존심", "qty": "만렙렙", "price": "지적 받으면 삐침","사과는 어려움"}
        ]
      },
      "detail_analysis": {
